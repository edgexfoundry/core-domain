...
public class Event extends BaseObject implements Serializable {
  ...
  /**
   * Set the pushed timestamp for this event and cascade the same timestamp to the associated
   * readings
   * 
   * @param pushed - timestamp
   */
  ...
  public String toString() {
    return "Event [pushed=" + pushed + ", device=" + device + ", readings=" + readings
        + ", toString()=" + super.toString() + "]";
  }

  @Override
  public int hashCode() {
    return new HashCodeBuilder().appendSuper(super.hashCode()).append(pushed).append(device)
        .append(readings).toHashCode();
  }
  ...
  private boolean propertyMatch(Event other) {
    if (pushed != other.pushed)
      return false;
    if (!stringPropertyMatch(device, other.device))
      return false;
    if (readings != null)
      return readings.equals(other.readings);
    else
      return (other.readings == null);
  }
