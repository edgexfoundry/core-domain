...
public class Reading extends BaseObject implements Serializable {
  ...
  public String toString() {
    return "Reading [pushed=" + pushed + ", name=" + name + ", value=" + value + ", device="
        + device + "]";
  }

  @Override
  public int hashCode() {
    return new HashCodeBuilder().appendSuper(super.hashCode()).append(pushed).append(name)
        .append(value).append(device).toHashCode();
  }
  ...
  private boolean propertyMatch(Reading other) {
    if (pushed != other.pushed)
      return false;
    if (!stringPropertyMatch(name, other.name))
      return false;
    if (!stringPropertyMatch(value, other.value))
      return false;
    return stringPropertyMatch(device, other.device);
  }
